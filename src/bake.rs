...
pub fn bake(
  ...
  let has_map_fn = fields.iter().filter_map(|f| f.map_fn.as_ref()).fold((false, false), |mut acc, m| {
    acc.0 |= m.view.is_some();
    acc.1 |= m.bake.is_some();
  ...
  let errors = fields
    ...
    .filter_map(|f| {
      ...
      if has_map_fn.0 && map_fn.view.is_none() {
        errors.push(darling::Error::missing_field("map_fn(view)").with_span(&map_fn.span()));
      }

      if has_map_fn.1 && map_fn.bake.is_none() {
        errors.push(darling::Error::missing_field("map_fn(bake)").with_span(&map_fn.span()));
      }
    ...
    .filter_map(
      |Field {
        ...
        let value = if let Some(map) = map {
          ...
          if let Some(view_spanned) = &map_fn.view {
    ...
    .filter_map(
      |Field {
        ...
        let value = if let Some(map) = map {
          ...
          if let Some(bake_spanned) = &map_fn.bake {
  ...
  let view = if view.len() == fields.len() {
    quote! {
      fn view<'__a>(&'__a self) -> #name_view #generics_view {
        #name_view {
          #( #view ),*
        }
      }
